#%% Import de libs necessárias
import requests
from bs4 import BeautifulSoup as bs
import pandas as pd

#%%
url = "https://www.vivareal.com.br/venda/parana/curitiba/?pagina={}"
    ...
    for house in houses:
        try:
            descricao = house.find(
                "span", {"class": "property-card__title"}
            ).text.strip()
        except:
            descricao = None
        try:
        ...
        try:
            area = house.find(
                "span", {"class": "property-card__detail-area"}
            ).text.strip()
        except:
            area = None
        try:
